...
export default function CreatePost({ onNeedSetup, onCreated }: Props) {
  ...
  const submit = async () => {
    ...
    setTitle(""); setDesc(""); setMediaUrl(""); setMediaPreview(""); setMediaType(undefined);
    onCreated();
  };

  const clearMedia = () => {
    setMediaUrl(""); setMediaPreview(""); setMediaType(undefined);
  };
    ...
    <div className="gradient-card border border-border rounded-xl p-4 glow-purple">
      ...
      {preview && (
        <div className="relative mb-2">
          {mediaType === "video" ? (
            <video src={preview} controls className="max-h-48 rounded-lg w-full object-contain bg-muted" />
          ) : (
            <img src={preview} alt="" className="max-h-48 rounded-lg w-full object-contain bg-muted" />
          )}
          {uploading && (
            <div className="absolute inset-0 bg-background/60 flex items-center justify-center rounded-lg">
              <Loader2 className="w-6 h-6 animate-spin text-primary" />
              <span className="ml-2 text-sm text-foreground">Uploading...</span>
            </div>
          )}
