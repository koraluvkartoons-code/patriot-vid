...
import { updatePost, togglePostLike, togglePinPost, archivePost } from "@/lib/api";
import UserBadge from "./UserBadge";
import CommentSection from "./CommentSection";
import { Heart, MessageCircle, Edit, ExternalLink, Pin, Archive } from "lucide-react";
    ...
    <div className={`gradient-card border rounded-xl overflow-hidden glow-purple transition-all hover:glow-pink ${post.isPinned ? "border-primary ring-1 ring-primary/30" : "border-border"}`}>
      ...
      {post.mediaUrl && (
        post.mediaType === "video" ? (
          ...
          <img src={post.mediaUrl} alt={post.title} className="w-full max-h-96 object-contain bg-background" loading="lazy" />
        )
      )}
      <div className="p-4">
            ...
            {(isOwner || isAdmin) && (
              <>
                ...
                <button onClick={handleArchive} title="Archive">
                  <Archive className="w-4 h-4 text-muted-foreground hover:text-destructive" />
                </button>
              </>
            )}
          </div>
